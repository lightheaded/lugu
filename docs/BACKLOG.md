...
| ~~Sleep timer service integration untested~~ | **Closed 27 August, and the seam named here the same day was then built.** The arithmetic was already covered, so the new tests took the number no test held: the volume at the tick that stops the book. `sleepTickFor` in `SleepFade.kt` now takes the per-tick decision — the volume to write, whether this tick stops the book, and the volume to leave behind — and the service only carries it out. So a test runs the whole loop with the player replaced by a list of writes, and asserts the thing that matters: the **last** write after a stop is 1.0, at every fade length the settings screen offers and at 0.5x, 1x, 1.8x and 3x. Without that write the player keeps the silence the fade ended on, and the next play makes no sound with nothing on screen to explain it. The restore was already correct in the service, and nothing could prove it. Behaviour is unchanged by the extraction |
