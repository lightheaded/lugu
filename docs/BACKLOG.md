...
| **A two-book library takes over a minute to mirror on a cold emulator** | Also found by the seeded CI server. The first resumption test timed out waiting 60s for `Lighthouse Wakes` to reach Room; the second, starting later, found it already there. The wait is now 180s so it is not the thing that fails, but a first sync of *two books* taking that long wants explaining — whether it is WorkManager scheduling, a cold emulator, or something that would also make a real sign-in feel broken |
