...
| Item | Why not yet |
|---|---|
| **Inline errors still reflow the forms and the player** | Left over from the 17 Aug status-line work, which took the sync, batch and failure lines out of the library grid and collections list. The same shape survives on the sign-in and connection screens, in the feedback form and in the player, where an error appears as a new line in a column and pushes everything under it down. A form is the one place the pattern is close to defensible — a message under the field it is about is where a reader looks for it — but the player is not a form, and neither is the feedback screen's send failure. Wants a decision about whether the status line generalises past the two screens it is on, which is why it was not done in the same pass |
...
| **"For you" in the car should hold what Continue holds** | Reported 17 Aug. lugu builds no node by that name — the root is one list in `BrowseTree.rootChildren` — so the section belongs to the host and the first task is finding out which host surface it is. If it is the recent/resumption root, it is ours and is roughly the Continue node under a different id; there are no recent-root or content-style hints in the service today. If it is Android Auto's own suggestions row, built from the system's history, there is no hook and the honest answer is that it cannot be changed from here. Keep the ask either way: *"I never use the car UI to discover what I'm going to listen next"* is an argument against spending anything on discovery surfaces in the car |
| **Next-in-series has never been watched working end to end** | Raised 17 Aug by Tom's own caveat on praising it. The path exists and its parts are tested — `nextInSeriesAfter` has a Room test, the series membership has migration tests, `DefaultContinuationResolver` applies the ask-first setting and cues rather than starts — but the join, a book reaching its end and the next one beginning, lives only as an unticked manual line in [qa/auto.md](qa/auto.md). Worth automating rather than ticking: a book can be seeked to its final seconds, so the check costs no more than any other instrumented test |
| **An episode page, with show notes** | Reported 17 Aug. There is no such page: the episode list draws a title and a subline, and the item page shows the *show's* description. An episode's own description is already mirrored into Room by the item sync and has never been drawn, so the show notes for every episode of every followed podcast are on the phone already, unread. Wants three decisions first — what a tap in the episode list means now that it unambiguously plays, how HTML show notes are rendered (the show description is drawn with a plain `Text` today, so a feed using `<p>` and `<a>` shows its tags), and where the page sits in the back stack when it is opened from a notification with the app not running |
| **The new-episode notification opens Home** | Reported 17 Aug, and the smaller half of the row above. `NewEpisodeNotifier` builds its tap action from `getLaunchIntentForPackage` — the bare launcher intent, carrying nothing about the episode that caused it — so the one thing the notification is about is lost at the moment it is acted on. Not fixable alone: it needs somewhere to land, which is the episode page |
