...
| **A fresh sign-in mirrors nothing until the Library tab is opened** | Found 16 Aug by the seeded CI server, and the first reading of it — "a slow mirror" — was wrong. The initial sync runs from `LibraryViewModel`'s init, and that view model is not built until the **Library tab** is composed. Signing in lands on **Home**. Measured: sitting on Home after a fresh sign-in, a two-book library never reached Room in **three minutes**; the next test, which restarted the app, found it there. So a new account has an empty Room until something opens the other tab — and everything that reads Room in between finds nothing: Home's own shelves, the car's browse tree, and `playFromSearch`, which is the path "play X on lugu" takes. A person can open the tab, which is what the instrumented test now does; a car cannot. Whether Home should trigger the first mirror itself is the decision to take |
...
| **A plain-HTTP server on the LAN cannot be reached at all** | Found 16 Aug while pointing the tests at a container. There is no `usesCleartextTraffic` and no network security config in the main manifest, so from Android 9 onwards every cleartext request is refused before it is made — and a great many self-hosters run Audiobookshelf over plain HTTP on their own network. The test builds now permit cleartext to `10.0.2.2` and `localhost` only, which is deliberately *not* a fix for this: the real answer is a decision about whether an address the user typed themselves may be trusted over HTTP, and probably a per-server opt-in rather than a blanket flag |
...
| Item | Note |
|---|---|
| ~~Process death itself is still not covered~~ | The second process exists: `:harness` is an application module with its own application id whose instrumented tests target *itself*, so it can end lugu's process and still be alive to say what happened. That much is proved on API 26 and API 36 emulators. What is **not** proved is the assertion the module was built for — the kill, the media button and the resumption are one test that needs a server, and no server has ever been pointed at it. See [qa/instrumented.md](qa/instrumented.md), including why the strict test kills the process rather than force-stopping it |
