...
| Item | Detail |
| --- | --- |
| **A deferred download delete leaks disk if the process dies** | Item 2 marks a completed download `pending_delete` and finalises it when the undo window closes. `finalizeDeferred` has exactly one caller: the undo coroutine in `ItemDetailViewModel`. So a process death inside the window, or a view model cleared when the listener leaves the page, strands the row. That state is invisible in three directions at once: `observeAll` and `observeForItem` exclude it, so no screen shows it; `unfinished()` excludes it, so the engine never touches it; and `pendingDeleteBytes()` is subtracted from the cap, so the space reads as free. The files stay on disk for ever. **Fix: sweep every `pending_delete` row at startup and finalise it.** `DownloadEngine.reconcile()` is the natural place, and `DownloadEntity` already carries `serverId` and `userId`, so the sweep needs no `ActiveAccount`. A circular dependency blocks the obvious call — `DownloadEngine` holds `DownloadDao` but not `DownloadRepository` — so the sweep either moves into the engine or gets called from the app's startup path |
