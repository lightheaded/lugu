...
| **The podcast preview does not draw its new play row** | `LibraryScreensScreenshotTest`'s podcast preview hand-builds the page, and the hero play button is composed inline in `ItemDetailScreen`. So the picture cannot show the button until that row becomes a composable of its own. The stale claim in the preview's doc comment is corrected; the picture is still incomplete |
