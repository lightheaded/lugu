...
| Item | Why not yet |
|---|---|
| **Inline errors still reflow the forms and the player** | **Decided and done 20 August, apart from one row kept below.** The decision the row asked for, made and written into the `StatusStrip` KDoc where the next person will reach for it: a message may appear, and nothing else may move. Which of the two mechanisms applies is decided by what the message is *about* and never by which screen it is on. A message about the screen, or about the outcome of an action, is an **overlay** — the strip in a `Box`, pinned under the top bar, holding no layout space — and that is what the player, the feedback form and the connection screen now use. A message about the input in front of you stays with that input in **space that is always reserved**: composed and hidden rather than added and removed, so the block is the same height in both states. The sign-in screen keeps its rejected-password line under the password box on those terms, and its plain-HTTP warning — a standing condition of the address rather than a message about an action — moves up under the address field with its space reserved and its wording untouched. A dialog was rejected: it stops a person to say something they can read in place. `StatusStrip` moved to a new `:core:ui` module on the way, because the feature modules are siblings and copying it would have been the `ContinueLabel` mistake again. Accessibility went with it: none of the four screens announced anything at all before, and every message that moved or gained reserved space is now a polite live region, with hidden text hidden from the screen reader too. **Closed 27 August.** The per-row download error keeps one line of space inside the row in every state — the list-row form of the same rule, and a third mechanism beside the overlay and the reserved field space, because an overlay under the top bar cannot say which of forty rows failed and a row has no field to sit under. The slot is one line of `labelSmall`, hidden with `reservedSpace` rather than sized in dp, so it holds at every font scale. It carries the failure's first sentence, cut to 48 characters at a word boundary with an ellipsis whenever anything was dropped; the cut is made in the text and not by `TextOverflow`, because a screen reader reads the whole string of a `Text` and a line that only looks short still reads out four sentences. The whole message reaches the screen's snackbar, which a tap on the row opens, and that channel's fixed 8 s became a floor that scales with word count to 24 s — the storage-cap message decided it, because its first sentence names the problem and the next three name the fix. The row itself is the polite live region, so a screen reader announces the title and the failure together: "Failed" alone names nothing in a list of forty rows. One cost, recorded in the code: a failed row with dropped words opens the message instead of the item page |
| ~~**The car's speed button does not say what speed it is on**~~ | **Fixed 27 August, and the diagnosis recorded here was wrong.** The work was never the plumbing and never the label. Android Auto draws a custom action as its **icon** alone, so a display name can never put text on a button, and that is why the first attempt changed nothing a driver could see. Media3 1.11.0 already ships the digits as vectors: `CommandButton.ICON_PLAYBACK_SPEED_0_5` through `_2_0`, where `media3_icon_playback_speed_1_2.xml` draws the glyphs `1`, `.`, `2` and `x` as paths, and `CommandButton.Builder(icon)` resolves the constant to a drawable id in this app's own resource table. `CarSpeedButton` maps the rate in force to that constant and holds one button per rate. **Keep this negative result, because it stops the obvious re-attempt:** a generated image was never reachable. A projection host is a legacy controller, and a `PlaybackStateCompat.CustomAction` carries an integer resource id in the app's own package and nothing else, so no run-time bitmap and no `content://` grant can reach a car button. Seven rates carry digits, so a rate off that set falls back to the plain icon — the player's fine adjustment moves in steps of 0.05 and reaches 1.35x, and a wrong number is worse than none. A new `Player.Listener` pushes the layout on every rate change, so the player screen, the notification and the car cannot disagree. The label names the rate in force, which is what Tom asked for. **Still open, and only a car answers it:** whether a head unit re-reads a custom action's icon on a pushed layout, or caches it from the first connection. That check is in [qa/auto.md](qa/auto.md) |
...
| ~~**Ask the server to fetch podcast episodes it does not have yet**~~ | **Done 27 August, and the missing button is the whole of what changed.** Tom asked directly Tom asked directly: *"I want to tell ABS server to download episodes that haven't been downloaded yet."* This is the **server-side** fetch — Audiobookshelf reading a podcast's RSS feed and pulling episodes into its own library folder — and not the phone-side download `:core:download` owns; the naming throughout keeps the two apart on purpose. The client and repository layers are done: `AbsPodcasts.kt` in `:core:api` wraps `GET /api/podcasts/:id/checknew`, `POST /api/podcasts/:id/download-episodes`, `GET /api/podcasts/:id/downloads`, `GET /api/podcasts/:id/clear-queue` and `POST /api/podcasts/feed`, all verified against the server source (no live server was reachable while this was built) rather than guessed at; `ServerEpisodeFetchRepository` in `:core:sync` wraps each call in a `Result`, matching `CollectionRepository`'s pattern for a shared, server-owned action with no offline retry queue — retrying a fetch request blindly risks a real duplicate on the server's side. The button is on the podcast page, under the play row, and it is drawn even when the page has nothing to play: a feed the server holds no episodes of is the feed that needs it most. It says "Get new episodes" and carries a feed icon, never a cloud, because the words and the picture keep the server-side fetch apart from the phone-side download. `ItemDetailViewModel.getNewEpisodes()` calls `checkAndFetchNewEpisodes(itemId)` and folds the `Result` into the page's own message channel. Four answers are covered, and "the feed has no new episodes" is one of them rather than a failure. While the request runs the icon becomes a spinner and the button refuses a second press, at the same height, so nothing moves under the finger. The outcome goes to the snackbar this page already uses for every other action, rather than to a `StatusStrip`: the backlog allowed either, and a second message mechanism on one screen is the worse of the two. The wiring as planned: a "Get new episodes" action on the podcast's own page (not a per-episode action — this asks the server about the whole feed), calling `checkAndFetchNewEpisodes(itemId)`, with the result surfaced through the screen's `StatusStrip` (an overlay, no layout shift) or a snackbar rather than a dialog — asking the server to fetch is not destructive, so it needs neither a confirmation nor an undo |
...
| ~~`EncryptedSharedPreferences` / `MasterKey` deprecated~~ | **Decided 27 August: keep it, and fix the failure path instead.** The library is deprecated as a whole — `javap` on the cached `.aar` shows class-level `java.lang.Deprecated` on both types — 1.1.0 is its last stable release, and AndroidX has no successor, so no version bump ends this. A plain `AndroidKeyStore` key is the eventual answer, and it moves key generation, the initialization vector, rotation and the invalidated-key case into the app. That last path had to work **before** a migration rather than after one, so it was built first, under the library that is here. `SecurePrefs` never throws at its caller. It repairs in two steps, a new file and then a new master key, and it keeps the stored bytes on a storage error, because a full disk must not cost a thirty-day session. **This fixed a live crash.** `tokens()` threw out of a lazy initializer, `AuthRepository.isSignedIn` called it, and `StartupViewModel` called that unguarded — so a device restore or a lock-screen change ended the app on the splash screen, on every launch, with no launch able to clear it. The sign-in screen now says why it asks, above the first field, and the wording lives in `:core:sync` beside the report that records the loss, because the settings screen asks for the same words. Storing the tokens unencrypted was rejected: a thirty-day refresh token and a certificate password are not a password somebody can re-type. Tink direct was rejected because only `tink-android` 1.8.0 is available to read, and that is below latest stable. **The move is forced by either of two events:** a `compileSdk` that breaks the library at run time, or an advisory against the unpinned transitive `tink-android` 1.8.0. The decision, the rejected options and the migration rule live in the KDoc on `EncryptedTokenStore` |
...
| ~~**The offline-first line is the next one worth automating, and was not**~~ | **Closed 27 August.** `OfflineLibraryTest` in `:harness` takes the line: sign in, wait for the title on the Library tab, `am force-stop`, cut the network, open lugu again, and the title must be back with no sign-in screen. The radio was the blocker, and the harness no longer touches it. `OfflineVpnService` opens a VPN that names lugu alone and never reads the tunnel, so lugu loses every route on API 26 and on API 36 by one route, and neither leg skips. The shell grants one app op, and the helper measures the consent with `VpnService.prepare` after each spelling of the command, rather than guessing from the API level. The rejected routes are recorded with their reasons: `cmd connectivity airplane-mode` starts at API 30; the older `settings put global airplane_mode_on` needs the protected `AIRPLANE_MODE` broadcast, which the platform takes from root and from the system and not from the shell, on API 26 as well; `svc wifi disable` leaves an emulator's mobile data up; and doze, battery saver and Data Saver all exempt the foreground app. The claim is narrower than airplane mode in three named ways, and it is safer, because only lugu enters the tunnel and the platform closes the tunnel with the harness process. See [qa/instrumented.md](qa/instrumented.md) |
...
| **Screenshot baselines cover components, not most screens** — **first screen closed 27 August** | 23 baselines in both themes, verified on every `./gradlew build`. Settings drives its real screen, and the Library tab now does as well: its content is `LibraryContent`, a stateless `internal` composable that takes plain state and plain callbacks, and the screenshot test calls it instead of arranging the screen's components itself. So a change to the order of the blocks in that file now fails a picture. Every other screen still takes a Hilt view model over final Room, DataStore and Ktor classes that cannot be faked, so those baselines remain pictures of each screen's own components in the order the screen uses. `LibraryContent` is the pattern for the rest |
...
| Sleep timer service integration untested — **narrowed 27 August** | The arithmetic was already covered, so `SleepTimerRestoreTest` took the number that no test held: the volume at the tick that stops the book. It runs the service loop with the player taken out. A twenty second fade is silent at that tick, at 1x and at 3x, so the player keeps silence unless something writes the volume back — and the next play makes no sound. A one second fade stops while the book can still be heard, and "no fade" stops at full volume, so the restore must be safe in both. **What is still not reached is the restore itself:** four lines inside private methods of `LuguPlaybackService`, which is a `MediaLibraryService` and needs a device. The seam that would close it: lift the per-tick decision — the volume to write, and whether this tick stops the book — into a pure function beside `SleepFade`, and let the service write what it returns. Then a test can assert that the last write before a stop is 1.0 |
...
| ~~**Item 4 names a player affordance that does not exist**~~ | **Built 27 August, so the plan text became true rather than wrong.** `PlayerActionRow` gains an "Up next" button, last in the row, because every other button there acts on the item playing now and this one opens what plays after it. It carries the icon and the words Home's top bar already uses for the same route, so one destination has one name. `PlayerScreen` takes `onOpenQueue` and threads it to the portrait layout and the landscape layout, which is what item 4 asked for. The button is always present, and that follows the rule the row already obeys: the transport row is configurable through `PlayerSettings.playerButtons`, the action row is not, and its one absence is a server capability rather than a preference. The four player baselines move for it |
| ~~**The mini player now has two code paths**~~ | **Unified 27 August, and the unification found a defect.** `MiniPlayer` now takes a `floor` slot and owns both rules the two paths held apart: the stacking order, and which side absorbs the gesture inset. Home hands its tab bar out through that slot and no longer knows that a mini player exists. The defect: a `Scaffold` with an empty bottom bar gives its content the system bar inset as padding, so on Home the shell spent the gesture inset and the tab bar had none left to absorb — it floated above a band of the shell's container colour. The shell now adds no modifier on the Home route, so Home draws as it did before the shell existed. The old `MiniPlayer` also emitted no node at all when nothing played, which is not the same to `Scaffold` as a bar of zero height, and that early return is kept on purpose |
| ~~**The player has no mini-player show and hide animation to keep**~~ | **Decided 27 August: no animation, and the promise is deleted from the plan.** The bar holds layout space, so an animated arrival reflows every screen under it once per frame, where a plain appearance reflows it once. That is what "a message may appear, and nothing else may move" forbids. Neither sanctioned mechanism fits: an overlay would cover content the bar must not cover, and reserved space would keep an empty band on every screen for a bar that is absent until somebody presses play. A pure opacity fade would obey the law, and it is still not worth it, because the bar appears in answer to a press. The reasoning sits in `MiniPlayer`'s KDoc so that nobody invents one later. The sentence "Keep the existing show and hide animation." is removed from item 1 of [UX-FIX-PLAN.md](UX-FIX-PLAN.md) |
| **One visible fix waits on the first real baseline record** | **Half done 27 August.** The Library search placeholder no longer promises four fields and draws one word: the label is "Search", the four fields go to the screen reader, and every label in that row ends with an ellipsis rather than a hard clip. **Still open:** the download control reads "Downloaded" rather than "Delete download" or "Cancel download", and only its content description was corrected. It lives in a photographed component, so the picture moves with the words |
| ~~**The podcast preview does not draw its new play row**~~ | **Done 27 August.** The hero play row is now `PodcastPlayRow` in `ItemDetailScreen.kt`, drawn exactly as before, and the preview calls it together with the new "Get new episodes" button. So the picture covers both controls the page leads with. The two podcast baselines, `item_podcast_light` and `item_podcast_dark`, wait on a Linux record |
| **`record-baselines.yml` has never run** | Written, never dispatched. Its artifact path glob and its `-Proborazzi.record` reach across modules were verified by reading, not by a run. **The blocker is a bootstrap step and not a design fault, and the first wording here read as if it were permanent.** GitHub offers a `workflow_dispatch` workflow from the **default** branch only, and `origin/main` still predates `d7d4738`, so the file is not there yet. One push of `main` fixes it for good, and dispatch then takes a ref, so `gh workflow run record-baselines.yml --ref <branch>` records for any branch. A route that needs no push exists and is proved: let the `build` job fail on the mismatch, run `gh run download <id> --name screenshot-diffs`, and commit the `*_actual.png` files from `<module>/build/outputs/roborazzi/`. Those bytes are the Linux runner's own render, and four player baselines went in that way and passed verification on the next run |
